...
def generate_data(
    n_qubits, n_gates, labels_as_described: bool, preprocessing_type: PreprocessingType
):
    X_train, Y_train = generate_dataset_ct(
        1,
        n_qubits,
        n_gates,
        labels_as_described=labels_as_described,
        preprocessing_type=preprocessing_type,
    )
    X_val, Y_val = generate_dataset_ct(
        1,
        n_qubits,
        n_gates,
        labels_as_described=labels_as_described,
    ...
    torch.save(
        (X_train, Y_train),
        f"train_data_{labels_as_described}_{preprocessing_type.value}.pt",
    )
    torch.save(
        (X_val, Y_val), f"val_data_{labels_as_described}_{preprocessing_type.value}.pt"
    )
...
if __name__ == "__main__":
    ...
    generate_data(
        n_qubits, n_gates_per_circuit, labels_as_described, preprocessing_type
    )

    labels_as_described = True
    preprocessing_type = PreprocessingType.FROM_PROJECT_DESCRIPTION
    generate_data(
        n_qubits, n_gates_per_circuit, labels_as_described, preprocessing_type
    )
    # main()
